...
int main(int argc, char *argv[])
{
    ...
    thrust::host_vector<thrust::tuple<unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned>> structForAgents;
    /*unique agent ID 0
    home partition ID 1
   ...
   /* for (int i = 0; i < IDs_helper.size(); i++)
    ...
    thrust::device_vector<unsigned> agentID, fillWithRank, fillIterNumber, countyID;
    agentID.reserve(NUM_OF_AGENTS * 1.2);
    agentID.resize(NUM_OF_AGENTS);
    fillWithRank.reserve(NUM_OF_AGENTS * 1.2);
    fillWithRank.resize(NUM_OF_AGENTS);
    fillIterNumber.reserve(NUM_OF_AGENTS * 1.2);
    fillIterNumber.resize(NUM_OF_AGENTS);
    countyID.reserve(NUM_OF_AGENTS * 1.2);
    countyID.resize(NUM_OF_AGENTS);
    structForAgents.reserve(NUM_OF_AGENTS * 1.2);
    ...
    //std::cout << rank << " sequence: " << agentID[0] << " "<<agentID[1]   << std::endl;
    thrust::fill(fillWithRank.begin(), fillWithRank.end(), urank);
    //std::cout << rank << " fillWithRank: " << fillWithRank[0] << " "<<fillWithRank[1]   << std::endl;
    thrust::fill(fillIterNumber.begin(), fillIterNumber.end(), NUM_OF_ITERATIONS);
    //std::cout << rank << " fillIterNumber: " << fillIterNumber[0] << " "<<fillIterNumber[1]   << std::endl;
    thrust::fill(countyID.begin(), countyID.end(), 0);
    //std::cout << rank << " countyID: " << countyID[0] << " "<<countyID[1]  << std::endl;
    ...
    thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple( // filling the struct with initial values
                     ...
                     thrust::make_zip_iterator(thrust::make_tuple(
                         ...
                         countyID.end(),
                         structForAgents.end())),
                     [] __host__ __device__(thrust::tuple<unsigned &, unsigned &, unsigned &, unsigned &, thrust::tuple<unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned> &> tup)
                     {
                         unsigned agentID_ = thrust::get<0>(tup);
                         unsigned fillWithRank_ = thrust::get<1>(tup);
                         unsigned fillIterNumber_ = thrust::get<2>(tup);
                         unsigned countyID_ = thrust::get<3>(tup);
                         thrust::get<4>(tup) = thrust::make_tuple<unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned>(agentID_, fillWithRank_, fillWithRank_, fillWithRank_, countyID_, fillIterNumber_, fillIterNumber_);
                     });
        ...
        { // if the process is dealing with these counties
            ...
            for (unsigned col_ind = csr_row[row_ind]; col_ind < csr_row[row_ind + 1]; col_ind++)
            {
                ...
                for (unsigned i=0;i<number_of_agents_to_this_col_ind;i++){
                    unsigned levaing_part_time,arriving_part_time;
                    int r= (rand() % NUM_OF_ITERATIONS/2);
                    int r2= (rand() % NUM_OF_ITERATIONS/2)+NUM_OF_ITERATIONS/2;
                    levaing_part_time = r;
                    arriving_part_time = r2;
                    /*if (i==0){
                        std::cout<<"rank"<< rank<< " dest_county" <<dest_county << " dest_partition " << dest_partition<<std::endl;
                    }*/


                    thrust::tuple<unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned> origAgent = structForAgents[processed_number_of_agents];
                    structForAgents[processed_number_of_agents]= thrust::make_tuple<unsigned, unsigned, unsigned, unsigned, unsigned, unsigned, unsigned>
                    (thrust::get<0>(origAgent), thrust::get<1>(origAgent), thrust::get<2>(origAgent), dest_partition, dest_county, levaing_part_time, arriving_part_time);
    ...
    for(unsigned  j =0;j<  structForAgents.size();j++){
        ...
        unsigned home =thrust::get<1>(structForAgents[j]);
        unsigned current =thrust::get<2>(structForAgents[j]);
        unsigned dest =thrust::get<3>(structForAgents[j]);
        unsigned county =thrust::get<4>(structForAgents[j]);
        unsigned leave =thrust::get<5>(structForAgents[j]);
        unsigned arrive =thrust::get<6>(structForAgents[j]);
        if (rank != dest)
            std::cout<<"rank"<< rank << " ID" <<id << " home " << home<< " current " << current<<  " dest" <<dest << " county " << county<< " leave " << leave<<"arrive"<< arrive<<std::endl;
        }
     ...
     if(rank == 0){
         std::cout << "used_parameters_NUM_OF_CITIES:" << NUM_OF_CITIES << "_NUM_OF_ITERATIONS:"<<NUM_OF_ITERATIONS2 << "_agents: "<< agents<<
